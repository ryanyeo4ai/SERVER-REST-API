...
def payment_request_history(reward_req_email):
    ...
    if len(records) > 0:
        for row in records:
            ...
            reward_payment_str = [date_time, email, account, payment_option, req_reward, req_status]
...
def extract_questions(src):
    ...
    # print(doc_list)
    conn, cursor = db_connect()
    sql = "select question_id, recognized_text from Question where question_id > 400"
    cursor.execute(sql)
    subjects_list = []
    subject_dict = {}
    records = cursor.fetchall()
...
def question_similarity_2(src_question):
    ...
    for i in range(len(sorted_cosine_scores_list)):
        if sorted_cosine_scores_list[1] == cosine_scores_list[i]:
            final_list.append(cosine_scores_list[i])
...
if __name__ == "__main__":
    ...
    sample_txt = "The price of Ken's sandwich"
    result, question_text_list = question_similarity_2(sample_txt) 
    print()
